...
function getDayName(date) {
  ...
  return options[new Date(dates).getUTCDay()];
}
...
function getNextFriday(date) {
  ...
  const newdate = curdate.setDate(
    date.getDate() + (((7 + 5 - date.getDay() - 1) % 7) + 1)
  );
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (
    Math.round(
      (new Date(dateEnd).getTime() - new Date(dateStart).getTime()) / oneDay
    ) + 1
  );
}
